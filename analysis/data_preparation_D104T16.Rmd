...
```{r}
create_obj(dir = "/Users/maria/OneDrive/Documentos/iMM/data/D104T61_forcedCells", 
           name = "D104T16_raw_srat.RDS")
```

```{r}
srat = readRDS("/Users/maria/OneDrive/Documentos/iMM/data/RDS/D104T16_raw_srat.RDS")
dim(srat)
```

```{r}
srat$orig.ident = "D104T16"
```
...
```{r}
# define thresholds
qcthr = c("maxMT" = 15, "minC" = 300, "minG" = 200, "maxC" = 10000, "maxG" = 3000)
...
```{r}
saveRDS(srat_filt, file = "/Users/maria/OneDrive/Documentos/iMM/data/RDS/D104T16_filt_srat.RDS")
```
...
```{r}
srat_filt = readRDS("/Users/maria/OneDrive/Documentos/iMM/analysis/results/RDS/D104T16_annot_srat_2.RDS")
dim(srat_filt)
```
...
```{r}
srat_filt = ScaleData(srat_filt, vars.to.regress = c("nCount_RNA", "nFeature_RNA",
...
FeaturePlot(srat_filt, features = c("Foxp3","Cxcr5"), order=T, pt.size = 1)
```
...
```{r}
cl_use = "SNN_pca30_res.0.4" 
DotPlot(srat_filt, features = c("Foxp3","Cxcr5","Ccr7","Klf2","S1pr1",
                                "Ctla4","Selplg", "Bcl6", "Pdcd1","Il2ra","Il1r2",
...
DimPlot(srat_filt, group.by = c("SNN_pca30_res.0.2", "SNN_pca30_res.0.3", "SNN_pca30_res.0.4"), 
        pt.size = 0.8, label = T, label.size = 12, ncol = 3)
```
...
```{r}
# sct normalization and scalling
# scale : nFeature, nCount, percent.mt, percent.rb
# 30 pca - res 0.4
cl_use = "SNN_pca30_res.0.4" 
srat_filt = SetIdent(srat_filt, value = cl_use)
sct_treg = WhichCells(srat_filt, idents = c("0","3","5"))
length(sct_treg)
```
...
cl_use = "SNN_pca30_res.0.4" 
srat_filt = SetIdent(srat_filt, value = cl_use)
srat_sct = subset(srat_filt, idents = c("0","3","5"))
```


```{r}
cl_use = "SNN_pca30_res.0.4" 
srat_sct = SetIdent(srat_sct, value = cl_use)
# remove cells Cxcr5 count above 1
sct_treg_cells = WhichCells(srat_sct, expression = Cxcr5 >= 1, invert = T) 
length(sct_treg_cells)
```
...
srat_filt = FindNeighbors(srat_filt, dims = 1:ncomp, force.recalc = T, verbose = T,
                          ...
                          graph.name = c(paste0("NN_", red, ncomp),
                                         paste0("SNN_", red, ncomp)))
# see how similar cells are to other cells in ncomp PC space
```
...
```{r fig.width=12, fig.height=5}
FeaturePlot(srat_filt, features = c("Foxp3","Cxcr5"), 
            order=T, pt.size = 1, ncol = 2)
```
...
```{r fig.width=6, fig.height=5}
DimPlot(srat_filt, group.by = c("SNN_pca20_res.0.6"), 
        pt.size = 0.8, label = T, label.size = 12, ncol = 1)
```



```{r fig.width=12, fig.height=10}
DimPlot(srat_filt, group.by = c("SNN_pca25_res.0.3","SNN_pca25_res.0.4",
                                "SNN_pca25_res.0.5","SNN_pca25_res.0.6"), 
        pt.size = 0.8, label = T, label.size = 12, ncol = 2)
```


```{r fig.width=18, fig.height=5}
FeaturePlot(srat_filt, features = c("Foxp3","Cxcr5","nCount_RNA"), 
...
cl_use = "SNN_pca20_res.0.6" 
srat_filt = SetIdent(srat_filt, value = cl_use)
mk_t16 = wilcoxauc(srat_filt, group_by = cl_use, seurat_assay = "RNA")
View(mk_t16)
```

```{r}
# normalization and scalling RNA
# scale : nFeature, nCount, percent.mt, percent.rb
# 25 pca - res 0.6
cl_use = "SNN_pca25_res.0.6" 
srat_filt = SetIdent(srat_filt, value = cl_use)
length(WhichCells(srat_filt, idents = c("0","2","5")))
```
...
cl_use = "SNN_pca20_res.0.6" 
srat_filt = SetIdent(srat_filt, value = cl_use)
length(WhichCells(srat_filt, idents = c("0","3","5")))
```
...
cl_use = "SNN_pca20_res.0.6" 
srat_filt = SetIdent(srat_filt, value = cl_use)
length(WhichCells(srat_filt, idents = c("0","3")))
```


```{r}
saveRDS(srat_filt, 
        file = "/Users/maria/OneDrive/Documentos/iMM/analysis/results/RDS/D104T16_annot_srat_2.RDS")
```
...
cl_use = "SNN_pca20_res.0.6" 
srat_filt = SetIdent(srat_filt, value = cl_use)
treg_cells = WhichCells(srat_filt, expression = Cxcr5 < 1, idents = c("0","3","5"))
length(treg_cells)
```
...
cl_use = "SNN_pca20_res.0.6" 
srat_filt = SetIdent(srat_filt, value = cl_use)
treg_cells = WhichCells(srat_filt, expression = Cxcr5 < 1, idents = c("0","3"))
length(treg_cells)
```


```{r}
write.csv(treg_cells, 
          file = "/Users/maria/OneDrive/Documentos/iMM/analysis/results/RDS/D104T16_treg_cellID.csv",
          row.names = F)
```
...
```{r}
srat_filt_treg = subset(srat_filt, cells = treg_cells)
dim(srat_filt_treg)
```

```{r}
length(intersect(colnames(srat_filt_treg), treg_cells))
```

```{r}
saveRDS(srat_filt_treg, 
        file = "/Users/maria/OneDrive/Documentos/iMM/analysis/results/RDS/D104T16_filt_treg_srat.RDS")
```
